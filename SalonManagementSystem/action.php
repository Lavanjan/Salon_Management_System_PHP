...
if (isset($_POST["action"])) {
    ...
    if ($_POST["action"] == "fetch") {
        $query = "SELECT * FROM styles ORDER BY id DESC";
  ...
  ';
    ...
    <tr>
     ...
     <td style="word-break: break-all;">' . $row["details"] . '</td>
...
//        mysqli_close($connect);
        ...
        $query = "INSERT INTO styles(image) VALUES ('$file')";
    ...
    if ($_POST["action"] == "update") {
        $file = addslashes(file_get_contents($_FILES["image"]["tmp_name"]));
        $query = "UPDATE styles SET image = '$file' WHERE id = '" . $_POST["image_id"] . "'";
        if (mysqli_query($connect, $query)) {
            echo 'Image Updated into Database';
        }
    }
    if ($_POST["action"] == "delete") {
        $query = "DELETE FROM styles WHERE id = '" . $_POST["image_id"] . "'";
        if (mysqli_query($connect, $query)) {
